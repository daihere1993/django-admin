...
function getCacheDate<T>(key: string, caches: CacheState<T>): T[] | boolean {
  const index = _.findIndex(Object.keys(caches), key);
  if (index !== -1) {
    return caches[key];
  }
  return false;
...
@Injectable()
export default class WordEffect {
  constructor(
  ...
  @Effect() loadWordGroup$: Observable<
    wordActions.LoadWordGroupSuccess
  > = this.actions$.pipe(
        ...
        return this.entityService
          ...
          .pipe(
            mergeMap((ret) => {
              // Update cache data
              return [
                new wordActions.UpdateWordListCache({
                  cache: ret.entities,
                  groupId,
                }),
  ...
  @Effect() addWord$: Observable<
    wordActions.AddWordSuccess
  > = this.actions$.pipe(
  ...
  @Effect() editWord$: Observable<
    wordActions.EditWordSuccess
  > = this.actions$.pipe(
  ...
  @Effect() addGroup$: Observable<
    wordActions.AddWordGroupSuccess
  > = this.actions$.pipe(
  ...
  @Effect() deleteWord$: Observable<
    wordActions.DeleteWordSuccess
  > = this.actions$.pipe(
